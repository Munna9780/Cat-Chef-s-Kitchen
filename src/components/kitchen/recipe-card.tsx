  ...
  return (
    ...
    >
      {recipe.image_url ? (
        ...
        <div className={`relative h-24 border-b-2 border-foreground ${accent}`}>
          <span className="absolute -bottom-7 right-5 grid size-20 place-items-center rounded-full border-2 border-foreground bg-background text-4xl transition-transform duration-200 group-hover:rotate-6 group-hover:scale-105" aria-hidden="true">{recipe.icon}</span>
        </div>
      )}
